...
(async () => {
  ...
  try {
    const token = require('jsonwebtoken').sign({ sub: '718e055f-af7f-40ac-93f4-3774fb724e62', role: 'SUPER_ADMIN' }, 'change-me-access-32-chars-min-xxxxx');
    ...
    await page.evaluate((jwtToken) => {
      localStorage.setItem('nuraskin-admin-auth', JSON.stringify({
        state: {
          token: jwtToken,
          user: {
    ...
    if (popoverCount > 0) {
      const isVisible = await page.locator('[data-radix-popper-content-wrapper]').first().isVisible();
