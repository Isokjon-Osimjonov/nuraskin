...
function CartPage() {
  ...
  let shipping = 0;
  if (cartRegion === 'KOR') {
    const matchedTier = shippingTiers.find(
      t => t.maxOrderKrw === null || subtotal < Number(BigInt(t.maxOrderKrw))
    );
  ...
  const minOrder =
    cartRegion === 'KOR' ? (settings?.minOrderAmountKrw ?? 0) : (settings?.minOrderAmountUzs ?? 0);
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center py-24 px-6">
        <h1 className="text-2xl font-normal text-[#4A1525] mb-3">
          Savatchani ko'rish uchun kiring
        </h1>
        <Link
          to="/login"
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center py-24 px-6">
        ...
        <p className="text-[14px] font-light text-stone-500 mb-8 max-w-md text-center">
          Siz hozircha hech qanday mahsulot tanlamadingiz. Katalogga o'tib, teriningiz uchun eng
          yaxshi variantlarni kashf eting.
        </p>
        <Link
          to="/products"
          className="bg-[#4A1525] text-white text-[13px] font-light tracking-wide px-7 py-3 rounded-full hover:bg-[#6B2540] transition-colors"
          ...
          <div className="w-full lg:w-2/3">
            <div className="bg-[#f8f7f5] rounded-2xl p-5 md:p-8">
              <div className="flex justify-between items-center border-b border-stone-200 pb-4 mb-6">
                ...
                <button
                  onClick={() => clearCart.mutate(cartRegion as string)}
                  className="text-[12px] font-light text-red-500 hover:text-red-600 tracking-wide"
                >
                  Barchasini tozalash
                </button>
              </div>
              ...
              <div className="space-y-5">
                {cart.map(item => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className="flex flex-col sm:flex-row items-start sm:items-center gap-5 py-4 border-b border-stone-200/50 last:border-0 last:pb-0"
                    >
                      {/* Image */}
                      <Link
                        to="/products/$slug"
                        params={{ slug: item.slug }}
                        className="w-20 h-20 sm:w-28 sm:h-28 rounded-xl bg-white overflow-hidden shrink-0"
                      >
                        <img
                      ...
                      <div className="flex-1 w-full">
                        <div className="flex justify-between items-start mb-2">
                          <Link
                            to="/products/$slug"
                            params={{ slug: item.slug }}
                            className="hover:opacity-70 transition-opacity pr-4"
                          >
                            <h3 className="text-[14px] font-light text-[#4A1525] leading-tight">
                              {item.productName}
                            </h3>
                          </Link>
                          <button
                        ...
                        <div className="text-[13px] font-light text-stone-500 mb-3 flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            ...
                            {isWholesale && (
                              <>
                                <span className="text-[10px] bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded border border-emerald-100 uppercase tracking-tighter font-normal italic">
                                  Ulgurji
                                </span>
                        ...
                        <div className="flex items-center justify-between w-full">
                          {/* Quantity */}
                          <div
                            className={`flex items-center justify-between w-24 h-9 border rounded-full px-3 bg-white ${isOverStock ? 'border-red-300 ring-1 ring-red-100' : 'border-stone-200'}`}
                          >
                            <button
                              onClick={() =>
                                updateQty.mutate({
                            ...
                            <span
                              className={`text-[13px] font-normal text-center ${isOverStock ? 'text-red-600' : ''}`}
                            >
                              {item.quantity}
                            </span>
                            <button
                              ...
                              className="text-stone-400 hover:text-[#4A1525] transition-colors disabled:opacity-50"
                              disabled={
                                updateQty.isPending || item.quantity >= (item.availableStock ?? 0)
                              }
                            >
              ...
              {cartRegion === 'UZB' && (
                <div className="mt-8 p-4 bg-emerald-50 rounded-xl border border-emerald-100 flex gap-3 items-start">
                  ...
                  <p className="text-[12px] text-emerald-700 leading-relaxed font-light">
                    <b>Yetkazib berish narxi mahsulot narxiga kiritilgan.</b> Siz alohida kargo
                    uchun to'lov qilmaysiz.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="w-full lg:w-1/3">
            <div className="bg-[#4A1525] text-white rounded-2xl p-7 sticky top-24">
              ...
              <div className="flex justify-between items-center border-t border-white/20 pt-5 mb-6">
                ...
                <span className="text-2xl font-normal tracking-tight">{displayPrice(total)}</span>
              </div>
