...
import { createAddressSchema, type CreateAddressInput, type AddressResponse } from '@nuraskin/shared-types';
import { MapPin, Phone, User, Tag, Loader2, Search, X, AlertCircle } from 'lucide-react';
...
export function AddressForm({ regionCode, initialData, onSubmit, onCancel, isSubmitting }: AddressFormProps) {
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label className="text-[12px] font-medium text-stone-500 ml-1">Manzil nomi (Masalan: Uy, Ish joyi)</label>
            ...
            {errors.label && <p className="text-[11px] text-red-500 ml-1">{errors.label.message}</p>}
          </div>

          <div className="space-y-1.5">
            <label className="text-[12px] font-medium text-stone-500 ml-1">Qabul qiluvchi F.I.SH</label>
            <div className="relative">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                {errors.uzbRegion && <p className="text-[11px] text-red-500 ml-1">{errors.uzbRegion.message}</p>}
              </div>
              <div className="space-y-1.5">
                ...
                />
                {errors.uzbCity && <p className="text-[11px] text-red-500 ml-1">{errors.uzbCity.message}</p>}
              </div>
            </div>
            <div className="space-y-1.5">
              <label className="text-[12px] font-medium text-stone-500 ml-1">Ko'cha va uy raqami</label>
              <input
                {...register('uzbStreet')}
                className="w-full h-11 px-4 rounded-xl bg-stone-50 border border-stone-100 text-[14px] outline-none focus:border-[#4A1525] transition-all"
                placeholder="Masalan: Navoiy ko'chasi, 15-uy"
              />
              {errors.uzbStreet && <p className="text-[11px] text-red-500 ml-1">{errors.uzbStreet.message}</p>}
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            {errors.korPostalCode && <p className="text-[11px] text-red-500 ml-1">{errors.korPostalCode.message}</p>}
            ...
            <div className="space-y-1.5">
              ...
              />
              {errors.korRoadAddress && <p className="text-[11px] text-red-500 ml-1">{errors.korRoadAddress.message}</p>}
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[12px] font-medium text-stone-500 ml-1">Bino nomi</label>
                <input
                  {...register('korBuilding')}
                  readOnly
                  className="w-full h-11 px-4 rounded-xl bg-stone-100 border border-stone-100 text-[14px] outline-none cursor-not-allowed"
                />
                {errors.korBuilding && <p className="text-[11px] text-red-500 ml-1">{errors.korBuilding.message}</p>}
              </div>
              <div className="space-y-1.5">
                <label className="text-[12px] font-medium text-stone-500 ml-1">Batafsil manzil (Kvartira, uy)</label>
                <input
                  {...register('korDetail')}
                  className="w-full h-11 px-4 rounded-xl bg-stone-50 border border-stone-100 text-[14px] outline-none focus:border-[#4A1525] transition-all"
                  placeholder="Masalan: 101-dong, 502-ho"
                />
                {errors.korDetail && <p className="text-[11px] text-red-500 ml-1">{errors.korDetail.message}</p>}
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3 pt-4 border-t border-stone-50">
          ...
          >
            {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : initialData ? "Yangilash" : "Saqlash"}
          </button>
        </div>
      </form>
