...
export function CouponFormPage() {
  ...
  const [selectedCustomers, setSelectedCustomers] = React.useState<
    { id: string; name: string; telegramUsername?: string }[]
  >([]);
  ...
  const customerScope =
    form.targetCustomerIds && form.targetCustomerIds.length > 0 ? 'SPECIFIC' : 'ALL';
    ...
    if (existingCoupon) {
      ...
      setForm({
        ...
        value:
          existingCoupon.type === 'PERCENTAGE'
            ? existingCoupon.value?.toString()
            : toUIValue(existingCoupon.value),
        value_uzs: toUIValue(existingCoupon.valueUzs, 'UZB'),
        value_krw: toUIValue(existingCoupon.valueKrw, 'KOR'),
        maxDiscountCap:
          existingCoupon.type === 'PERCENTAGE' ? toUIValue(existingCoupon.maxDiscountCap) : '',
        ...
        startsAt: existingCoupon.startsAt
          ? new Date(existingCoupon.startsAt).toISOString().split('T')[0]
          : '',
      ...
      if (existingCoupon.applicableResourceIds?.length) {
        if (existingCoupon.scope === 'PRODUCTS') {
          Promise.all(
            existingCoupon.applicableResourceIds.map((id: string) => api.get(`/products/${id}`))
          ).then(res =>
            setSelectedProducts(
              res.map((p: any) => ({ id: p.id, name: p.name, barcode: p.barcode }))
            )
          );
        } else if (existingCoupon.scope === 'CATEGORIES') {
          Promise.all(
            existingCoupon.applicableResourceIds.map((id: string) =>
      ...
      if (existingCoupon.targetCustomerIds?.length) {
        Promise.all(
          existingCoupon.targetCustomerIds.map((id: string) => api.get(`/customers/${id}`))
        ).then(res =>
      ...
      const payload = {
        ...
        value:
          data.type === 'PERCENTAGE'
            ...
            : region === 'ALL'
              ? '0'
              : toDbValue(data.value),
        valueUzs:
          region === 'ALL' && data.type !== 'PERCENTAGE' ? toDbValue(data.value_uzs, 'UZB') : null,
        valueKrw:
          region === 'ALL' && data.type !== 'PERCENTAGE' ? toDbValue(data.value_krw, 'KOR') : null,
        maxDiscountCap:
          region === 'ALL'
            ? null
            : data.type === 'PERCENTAGE'
              ? toDbValue(data.maxDiscountCap)
              : null,
        maxDiscountUzs:
          region === 'ALL' && data.type === 'PERCENTAGE'
            ? toDbValue(data.max_discount_uzs, 'UZB')
            : null,
        maxDiscountKrw:
          region === 'ALL' && data.type === 'PERCENTAGE'
            ? toDbValue(data.max_discount_krw, 'KOR')
            : null,
    ...
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Chegirma sozlamalari</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    value={form.type}
                    onValueChange={v =>
                      setForm({ ...form, type: v, value: v === 'FREE_SHIPPING' ? '0' : form.value })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Qo'llanish doirasi</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    {selectedProducts.map(p => (
                      <div
                        key={p.id}
                        className="flex items-center gap-2 bg-pink-50 text-pink-800 px-3 py-1.5 rounded-full text-xs border border-pink-100"
                      >
              ...
              {form.scope === 'CATEGORIES' && (
                <div className="space-y-3 pt-2">
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            if (isSelected) {
                              setSelectedCategories(
                                selectedCategories.filter(s => s.id !== cat.id)
                              );
                            } else {
                              setSelectedCategories([
                                ...selectedCategories,
                  ...
                  {selectedCategories.length > 0 && (
                    <p className="text-[11px] text-[#E30B5C] font-medium">
                      {selectedCategories.length} ta kategoriya tanlandi
                    </p>
                  )}
                </div>
              )}
                ...
                <div className="space-y-3 pt-2">
                            ...
                            return (
                              <div
                                ...
                                className={`px-4 py-3 border-b border-stone-50 last:border-0 flex justify-between items-center transition-colors ${
                                  isSelected
                                    ? 'bg-stone-50 opacity-60 cursor-default'
                              ...
                              >
                                <div>
                                  <div className="text-sm font-medium text-[#3A0311]">
                                    {c.fullName || 'Nomsiz'}
                                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    {selectedCustomers.map(c => (
                      ...
                      >
                        ...
                        {c.telegramUsername && (
                          <span className="text-pink-400 opacity-80">@{c.telegramUsername}</span>
                        )}
                        <button
                          type="button"
                          onClick={() =>
                  ...
                  {selectedCustomers.length === 0 && (
                    <p className="text-[11px] text-stone-400 italic">
                      Hech kim tanlanmagan — barcha mijozlarga qo'llanadi
                    </p>
                  )}
                </div>
              )}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Shartlar</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                {form.regionCode === 'ALL' ? (
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Bu kuponni storefront sahifasida reklama sifatida ko'rsatish
                  </p>
                </div>
                <Switch
                ...
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[10px] text-muted-foreground italic">
                      Bo'sh qoldirilsa kupon nomi ishlatiladi
                    </p>
                  </div>
                </>
              )}
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Vaqt va Limit</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Holat</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>Status</Label>
