...
import { ArrowLeft, ArrowRight, Send, Sparkles, Loader2, Camera, Check } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-10 gap-8 items-start">
        ...
        <div className="lg:col-span-6 space-y-6">
                      ...
                      {form.imageUrls[0] === url && (
                        <div className="absolute bottom-1 left-1">
                          <Badge variant="secondary" className="text-[8px] px-1 py-0">
                            <Camera className="w-2 h-2 mr-1" /> Asosiy
                          </Badge>
                        </div>
                      )}
                    </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      value={form.postType}
                      onValueChange={v => setForm({ ...form, postType: v })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
          ...
          {step === 4 && (
            <Card>
              <CardHeader>
                <CardTitle>Narx sozlamalari</CardTitle>
              </CardHeader>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-3">
                      ...
                      {fullProduct.regionalConfigs?.find((c: any) => c.regionCode === 'KOR') ? (
                        (() => {
                          const korConfig = fullProduct.regionalConfigs.find(
                            (c: any) => c.regionCode === 'KOR'
                          );
                          if (!korConfig)
                            return (
                              <p className="text-xs text-stone-400 italic">
                                Koreya narxi topilmadi
                              </p>
                            );
                          return (
                            <>
                              <div className="flex items-center justify-between">
                                ...
                                <Switch
                                  checked={form.showKrwRetail}
                                  onCheckedChange={v => setForm({ ...form, showKrwRetail: v })}
                                />
                              </div>
                              <div className="flex items-center justify-between">
                                <div className="space-y-0.5">
                                  ...
                                  <p className="text-xs text-muted-foreground">
                                    ...
                                    dan — {korConfig.minWholesaleQty || 5} tadan
                                  </p>
                                </div>
                                <Switch
                                  checked={form.showKrwWholesale}
                                  onCheckedChange={v => setForm({ ...form, showKrwWholesale: v })}
                                />
                              </div>
                            </>
                          );
                        ...
                        <p className="text-xs text-stone-400 italic">
                          Koreya uchun narx kiritilmagan
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      {fullProduct.regionalConfigs?.find((c: any) => c.regionCode === 'UZB') &&
                      latestRate ? (
                        (() => {
                          const uzbConfig = fullProduct.regionalConfigs.find(
                            (c: any) => c.regionCode === 'UZB'
                          );
                          if (!uzbConfig)
                            return (
                              <p className="text-xs text-stone-400 italic">
                                O'zbekiston narxi topilmadi
                              </p>
                            );
                          ...
                          return (
                            <>
                              <div className="flex items-center justify-between">
                                ...
                                <Switch
                                  checked={form.showUzsRetail}
                                  onCheckedChange={v => setForm({ ...form, showUzsRetail: v })}
                                />
                              </div>
                              <div className="flex items-center justify-between">
                                <div className="space-y-0.5">
                                  <Label>🇺🇿 Optom narxi</Label>
                                  <p className="text-xs text-muted-foreground">
                                    {formatUzs(wholesaleUzs)} dan — {uzbConfig.minWholesaleQty || 5}{' '}
                                    tadan
                                  </p>
                                </div>
                                <Switch
                                  checked={form.showUzsWholesale}
                                  onCheckedChange={v => setForm({ ...form, showUzsWholesale: v })}
                                />
                              </div>
                            </>
                          );
                        ...
                        <p className="text-xs text-stone-400 italic">
                          O'zbekiston narxi yoki kurs topilmadi
                        </p>
                      )}
                    </div>
                  </div>
          ...
          {step === 5 && (
            <Card>
              <CardHeader>
                <CardTitle>Havolalar va Kontaktlar</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      checked={form.showAdminPhone}
                      onCheckedChange={v => setForm({ ...form, showAdminPhone: v })}
                    />
                  </div>
                  {form.showAdminPhone && (
                    <Input
                ...
                <div className="pt-4 border-t space-y-4">
                  ...
                  <div className="space-y-2 p-3 border rounded-lg bg-stone-50/50">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Switch
                        checked={form.link1Show}
                        onCheckedChange={v => setForm({ ...form, link1Show: v })}
                      />
                    </div>
                    {form.link1Show && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <Input
                          placeholder="Matn (Masalan: Telegram)"
                          value={form.link1Text}
                          onChange={e => setForm({ ...form, link1Text: e.target.value })}
                        />
                        <Input
                          placeholder="URL (https://...)"
                          value={form.link1Url}
                          onChange={e => setForm({ ...form, link1Url: e.target.value })}
                        />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2 p-3 border rounded-lg bg-stone-50/50">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Switch
                        checked={form.link2Show}
                        onCheckedChange={v => setForm({ ...form, link2Show: v })}
                      />
                    </div>
                    {form.link2Show && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <Input
                          placeholder="Matn (Masalan: Instagram)"
                          value={form.link2Text}
                          onChange={e => setForm({ ...form, link2Text: e.target.value })}
                        />
                        <Input
                          placeholder="URL (https://...)"
                          value={form.link2Url}
                          onChange={e => setForm({ ...form, link2Url: e.target.value })}
                        />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2 p-3 border rounded-lg bg-stone-50/50">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Switch
                        checked={form.link3Show}
                        onCheckedChange={v => setForm({ ...form, link3Show: v })}
                      />
                    </div>
                    {form.link3Show && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <Input
                          placeholder="Matn (Masalan: Website)"
                          value={form.link3Text}
                          onChange={e => setForm({ ...form, link3Text: e.target.value })}
                        />
                        <Input
                          placeholder="URL (https://...)"
                          value={form.link3Url}
                          onChange={e => setForm({ ...form, link3Url: e.target.value })}
                        />
                      </div>
                    )}
                  </div>
          ...
          {step === 6 && (
            <Card>
              <CardHeader>
                <CardTitle>Kanallar va Vaqt</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {channels.map(c => (
                      <label
                        key={c.id}
                        className="flex items-center gap-3 p-3 rounded-xl border hover:bg-muted/50 cursor-pointer"
                      >
                        <Checkbox
                ...
                <div className="pt-4 border-t space-y-4">
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(v: string) =>
                      setForm({ ...form, scheduledAt: v === 'NOW' ? '' : new Date().toISOString() })
                    }
                  >
                    <div className="flex items-center space-x-2">
          ...
          {step === 7 && (
            <Card>
              ...
              <CardFooter className="flex gap-3">
                ...
                <Button
                  className="flex-1"
                  onClick={() => createMutation.mutate(form.scheduledAt ? 'SCHEDULED' : 'SENT')}
                  disabled={createMutation.isPending}
                >
                  {createMutation.isPending ? (
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
        ...
        <div className="lg:col-span-4 sticky top-24">
          <p className="text-[10px] uppercase font-bold text-muted-foreground mb-3 tracking-widest text-center">
            Live Preview
          </p>
