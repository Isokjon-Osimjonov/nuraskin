    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto w-full min-w-0">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-muted-foreground">Post yuboriladigan kanallar va guruhlar</p>
        </div>
            ...
            {isLoading ? (
              ...
              channels.map((channel: any) => (
                <DataTableRow key={channel.id}>
                  ...
                  <DataTableCell>
                    <Badge variant="outline" className="text-[10px] tracking-wide rounded-full">
                      {channel.chatType}
                    </Badge>
                  </DataTableCell>
                  ...
                  <DataTableCell className="text-right">
                    <Button
                      ...
                      onClick={() => {
                        if (confirm("Kanalni o'chirmoqchimisiz?"))
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              {testResult && (
                <div
                  className={`p-3 rounded-md text-sm flex items-start gap-2 ${testResult.ok ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}
                >
                  {testResult.ok ? (
                  ...
                  <div>
                    <p className="font-medium">
                      {testResult.ok ? 'Ulanish muvaffaqiyatli' : 'Xatolik'}
                    </p>
                    <p className="text-xs opacity-90">
                      {testResult.ok ? `Kanal: ${testResult.title}` : testResult.error}
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={v => setNewChannel(p => ({ ...p, language: v }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={newChannel.chatType}
                  onValueChange={v => setNewChannel(p => ({ ...p, chatType: v as any }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !newChannel.name || !newChannel.chatId || !testResult?.ok || addMutation.isPending
              }
            >
