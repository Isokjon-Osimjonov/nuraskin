...
export async function sendToAdmin(message: string): Promise<void> {
  if (!env.TELEGRAM_ADMIN_BOT_TOKEN || !env.TELEGRAM_ADMIN_CHAT_ID) {
    logger.warn('TELEGRAM_ADMIN_BOT_TOKEN or TELEGRAM_ADMIN_CHAT_ID not set — admin notifications disabled');
    return;
  }
...
export async function sendToCustomer(telegramId: number | string | bigint, message: string): Promise<void> {
  ...
  try {
    ...
    logger.error({ error, telegramId: telegramId.toString() }, 'Failed to send customer Telegram message');
  }
}
