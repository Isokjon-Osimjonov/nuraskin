...
export async function validateAndApply(
  ...
  if (coupon.scope === 'PRODUCTS' && coupon.applicableResourceIds) {
    ...
    applicableItems = cartItems.filter(
      i => i.brandName && coupon.applicableBrands!.includes(i.brandName)
    );
  }
  ...
  if (applicableItems.length === 0) {
    throw new CouponNotApplicableError(
      'Kupon savatchangizdagi mahsulotlarga mos kelmadi yoki ular ulgurji narxda'
    );
  }
  ...
  if (coupon.type === 'PERCENTAGE') {
    ...
    if (coupon.regionCode === 'ALL') {
      baseValue =
        regionCode === 'UZB' ? BigInt(coupon.valueUzs || 0) : BigInt(coupon.valueKrw || 0);
    } else {
    ...
    if (coupon.regionCode === 'ALL') {
      maxCap =
        regionCode === 'UZB'
          ? coupon.maxDiscountUzs
            ? BigInt(coupon.maxDiscountUzs)
            : null
          : coupon.maxDiscountKrw
            ? BigInt(coupon.maxDiscountKrw)
            : null;
    } else {
...
export async function updateCoupon(id: string, input: UpdateCouponInput) {
  ...
  if (existing.status !== 'DRAFT' && existing.status !== 'PAUSED') {
    ...
    const hasSignificantChanges = nonStatusKeys.some(k => {
      ...
      if (
        (k === 'isPromotional' || k === 'isFirstPurchaseOnly') &&
        val === false &&
        !(existing as any)[k]
      )
        return false;
  ...
  if (input.valueUzs !== undefined)
    updateData.valueUzs = input.valueUzs ? BigInt(input.valueUzs) : null;
  if (input.valueKrw !== undefined)
    updateData.valueKrw = input.valueKrw ? BigInt(input.valueKrw) : null;
  if (input.maxDiscountCap !== undefined)
    updateData.maxDiscountCap = input.maxDiscountCap ? BigInt(input.maxDiscountCap) : null;
  if (input.maxDiscountUzs !== undefined)
    updateData.maxDiscountUzs = input.maxDiscountUzs ? BigInt(input.maxDiscountUzs) : null;
  if (input.maxDiscountKrw !== undefined)
    updateData.maxDiscountKrw = input.maxDiscountKrw ? BigInt(input.maxDiscountKrw) : null;
  if (input.minOrderAmount !== undefined)
    updateData.minOrderAmount = BigInt(input.minOrderAmount || 0);
  if (input.minOrderUzs !== undefined)
    updateData.minOrderUzs = input.minOrderUzs ? BigInt(input.minOrderUzs) : null;
  if (input.minOrderKrw !== undefined)
    updateData.minOrderKrw = input.minOrderKrw ? BigInt(input.minOrderKrw) : null;
...
export async function recordRedemption(
  data: { couponId: string; customerId: string; orderId: string; discountAmount: bigint },
  tx: any
) {
